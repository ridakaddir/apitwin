#!/usr/bin/env bash
# run.sh — QA harness orchestrator for gRPC response-wrapped auto-derive.
# Structure mirrors qa/grpc-transitions-persist/run.sh so iteration reports
# and scenario filters behave identically.
set -euo pipefail
...
PROTO_FILE="$HARNESS_DIR/countries.proto"
...
start_server() {
  ...
  {
    echo "----"
    echo "$(date -u +%FT%TZ) starting server grpc_port=$GRPC_PORT http_port=$HTTP_PORT binary=$BINARY"
  } >> "$LOG_FILE"
  ...
  # This harness exists to regression-test gRPC auto-derive persist
  # semantics — not the runtime dir feature — so use the legacy path.
...
# Pre-flight start to catch config errors.
reset_stubs
if ! start_server; then
  jq -nc --arg id "00-startup" --arg status "fail" \
  ...
  jq -n --argjson iteration "$ITER" --arg binary "$BINARY" --argjson grpc_port 0 \
    --arg started_at "$STARTED_AT" --argjson total 1 --argjson passed 0 --argjson failed 1 \
...
for sf in "${SCENARIO_FILES[@]}"; do
  ...
  reset_stubs
  if ! start_server; then
    jq -nc --arg id "$sname" --arg status "fail" \
...
jq -n --argjson iteration "$ITER" --arg binary "$BINARY" --argjson grpc_port "${GRPC_PORT:-0}" \
  --arg started_at "$STARTED_AT" --argjson total "$TOTAL" --argjson passed "$PASSED" --argjson failed "$FAILED" \
  --argjson scenarios "$SCENARIOS_JSON" \
  '{iteration:$iteration,binary:$binary,grpc_port:$grpc_port,started_at:$started_at,total:$total,passed:$passed,failed:$failed,scenarios:$scenarios}' \
  > "$REPORT_FILE"
