...
1. apitwin reads the defaults file and resolves [template tokens](template-tokens.md) (<code v-pre>{{uuid}}</code> becomes a real UUID, <code v-pre>{{now}}</code> becomes a timestamp)
