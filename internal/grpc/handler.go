...
func (h *handler) serve(srv interface{}, stream grpc.ServerStream) error {
	...
	for i := range cfg.GRPCRoutes {
		...
		if c.Persist {
			grpcCode, ok, persistResult, persistedPath := h.applyGRPCPersist(c, reqMap, start, fullMethod, md)
			if ok {
				...
				if grpcCode == codes.NotFound && isTransitionCase {
					if fc, fbOK := route.Cases[route.Fallback]; fbOK && fc.Persist {
						grpcCode, ok, persistResult, persistedPath = h.applyGRPCPersist(fc, reqMap, start, fullMethod, md)
						if !ok {
							break
						}
					}
